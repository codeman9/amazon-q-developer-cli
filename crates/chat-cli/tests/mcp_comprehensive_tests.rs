...
fn test_mock_server_functionality() {
    let server = MockMcpServer::new("weather-server").with_tool("get_weather", "Get current weather", vec![
    ...
    // Test failing server
    let failing_server = MockMcpServer::new("failing-server").with_failure("Test failure");
...
async fn test_configuration_parsing() {
    ...
    let result = discovery_service
        .discover_servers_from_path(&minimal_path, "test")
        .await
        .unwrap();
    ...
    let result = discovery_service
        .discover_servers_from_path(&non_existent, "test")
        .await;
    ...
    let result = discovery_service
        .discover_servers_from_path(&empty_config_path, "test")
        .await
        .unwrap();
...
async fn test_data_consistency() {
    ...
    let result1 = discovery_service
        .discover_servers_from_path(&config_path, "test")
        .await
        .unwrap();
    let result2 = discovery_service
        .discover_servers_from_path(&config_path, "test")
        .await
        .unwrap();
...
fn test_tool_schema_edge_cases() {
    ...
    let contexts = processor
        .create_mcp_contexts(&server_info, &empty_tools_result)
        .unwrap();
