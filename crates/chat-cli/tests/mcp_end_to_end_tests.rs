...
use chat_cli::util::{
    ...
    test_utils::{create_mock_servers, create_temp_mcp_config, MockMcpServer},
};
...
async fn test_complete_mcp_rag_workflow() {
    ...
    let query_tools = integration_service.get_mcp_tools_for_query("weather", Some(5)).await.unwrap();
    ...
    let mcp_tools: Vec<_> = integrated_config.values()
...
async fn test_mcp_performance() {
    ...
    for server in &servers[..5] { // Test with first 5 servers
        let server_info = server.to_server_info();
        let tools_result = server.get_tools_result().unwrap();
        let contexts = processor.create_mcp_contexts(&server_info, &tools_result).unwrap();
