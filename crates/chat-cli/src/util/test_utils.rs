...
impl PerformanceTestUtils {
        ...
        (0..count)
            .map(|i| {
                MockMcpServer::new(&format!("server-{}", i))
                    ...
                    .with_tool(
                        &format!("tool-{}-2", i),
                        &format!("Second tool for server {}", i),
                        vec![("data", "object", true)],
                    )
            })
            .collect()
    }
    ...
    pub fn create_varied_servers() -> Vec<MockMcpServer> {
        ...
        for i in 0..5 {
            servers.push(MockMcpServer::new(&format!("small-server-{}", i)).with_tool(
                &format!("tool-{}", i),
                "Simple tool",
                vec![("param", "string", true)],
            ));
        }
...
mod tests {
    ...
    fn test_mock_server_creation() {
        let server =
            MockMcpServer::new("test-server").with_tool("test_tool", "A test tool", vec![("param", "string", true)]);
    ...
    fn test_failing_server() {
        let server = MockMcpServer::new("failing-server").with_failure("Test failure");
