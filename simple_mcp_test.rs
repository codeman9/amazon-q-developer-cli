...
use chat_cli::util::mcp_tool_integration::McpToolIntegrationService;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match integrate_mcp_tools_into_config(base_config).await {
        Ok(integrated_config) => {
            ...
            let native_tools = integrated_config
                .values()
                .filter(|spec| spec.tool_origin == ToolOrigin::Native)
                .count();
            let mcp_tools = integrated_config
                .values()
